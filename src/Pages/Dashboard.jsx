...
const Dashboard = () => {
  ...
  useEffect(() => {
    console.log(Auth);
    axios.get(`${backend}api/orders`).then((res) => {
      setOrders(res.data);
  ...
  const deleteOrder = (id) => {
    ...
    axios.post(`${backend}api/orders/remove/${id}`).then((res) => {
      setTemp(!temp);
    });
  };
  const updateOrderStatus = (id, status) => {
    ...
    axios
      .post(`${backend}api/orders/update-status/${id}/${status}`)
      .then((res) => {
        setTemp(!temp);
        setFiltered(false);
      });
      ...
      (Auth.user.type === "deliveryManager" ||
        ...
        <div className="orders">
          ...
          <div className="header">
            <h1 className="cat-text content__padding">Nouwakchott</h1>
          </div>
          <>
            <div
